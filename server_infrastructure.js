...
// Storage layout
const DATA_ROOT = path.join(process.cwd(), "uploads", "infrastructure");
...
async function ensureSchema() {
  ...
  `);
  await pool.query(`
    INSERT INTO inf_settings (id) VALUES (1) ON CONFLICT (id) DO NOTHING;
  `);
  ...
  `);
  // Add missing columns
  try { await pool.query(`ALTER TABLE inf_categories ADD COLUMN IF NOT EXISTS sort_order INT NOT NULL DEFAULT 0;`); } catch {}
  try { await pool.query(`ALTER TABLE inf_categories ADD COLUMN IF NOT EXISTS marker_size INT NOT NULL DEFAULT 32;`); } catch {}
  try { await pool.query(`ALTER TABLE inf_categories ADD COLUMN IF NOT EXISTS assign_to_controls BOOLEAN NOT NULL DEFAULT false;`); } catch {}

  // Items (similar to equipments but simpler)
  await pool.query(`
    CREATE TABLE IF NOT EXISTS inf_items (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      name TEXT NOT NULL,
      code TEXT,
      category_id UUID REFERENCES inf_categories(id) ON DELETE SET NULL,
      building TEXT,
      floor TEXT,
      location TEXT,
      description TEXT,
  ...
  `);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS code TEXT;`);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS description TEXT;`);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS notes TEXT;`);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS data JSONB DEFAULT '{}'::jsonb;`);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS photo_file_id UUID;`);

  // Fire interlock columns - link to fire control system
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS fire_interlock BOOLEAN DEFAULT FALSE;`);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS fire_interlock_zone_id UUID;`);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS fire_interlock_alarm_level INTEGER DEFAULT 1;`);
  await pool.query(`ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS fire_interlock_code TEXT;`);

  // Add created_by columns for ownership tracking
  await pool.query(`
    DO $$ BEGIN
      ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS created_by_email TEXT;
    EXCEPTION WHEN duplicate_column THEN NULL; END $$;
  `);
  await pool.query(`
    DO $$ BEGIN
      ALTER TABLE inf_items ADD COLUMN IF NOT EXISTS created_by_name TEXT;
    EXCEPTION WHEN duplicate_column THEN NULL; END $$;
  `);

  // Files attached to items
  await pool.query(`
    CREATE TABLE IF NOT EXISTS inf_files (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      item_id UUID REFERENCES inf_items(id) ON DELETE CASCADE,
  ...
  `);
  await pool.query(`ALTER TABLE inf_files ADD COLUMN IF NOT EXISTS content BYTEA;`);
  await pool.query(`ALTER TABLE inf_files ADD COLUMN IF NOT EXISTS kind TEXT DEFAULT 'file';`);
  try { await pool.query(`ALTER TABLE inf_files ALTER COLUMN filepath DROP NOT NULL;`); } catch {}

  // Positions on VSD maps (uses VSD plans - same as mobile equipment)
  await pool.query(`
    CREATE TABLE IF NOT EXISTS inf_positions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      item_id UUID REFERENCES inf_items(id) ON DELETE CASCADE,
  ...
  // This ensures infrastructure can work even if VSD microservice hasn't run yet
  await pool.query(`
  ...
  console.log("[Infrastructure] Schema ready (including VSD plans tables)");
}

// Create audit trail helper
const audit = createAuditTrail(pool, "infrastructure");
...
// HEALTH CHECK
// ====================
app.get("/api/infrastructure/health", (_req, res) => res.json({ ok: true, service: "infrastructure" }));
...
// List categories
app.get("/api/infrastructure/categories", async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT c.*,
             (SELECT COUNT(*) FROM inf_items i WHERE i.category_id = c.id) as item_count
        FROM inf_categories c
       ORDER BY c.sort_order, c.name
    `);
    res.json({ ok: true, categories: rows });
  } catch (e) {
    console.error("[Infrastructure] List categories error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Create category
app.post("/api/infrastructure/categories", async (req, res) => {
  try {
    const { name, description, color, icon, marker_size, sort_order, assign_to_controls } = req.body;
    if (!name?.trim()) return res.status(400).json({ ok: false, error: "Name required" });

    const { rows } = await pool.query(`
      INSERT INTO inf_categories (name, description, color, icon, marker_size, sort_order, assign_to_controls)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *
    `, [name.trim(), description || null, color || '#8B5CF6', icon || 'building', marker_size || 32, sort_order || 0, assign_to_controls || false]);
    ...
    console.error("[Infrastructure] Create category error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Update category
app.put("/api/infrastructure/categories/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, color, icon, marker_size, sort_order, assign_to_controls } = req.body;

    const { rows } = await pool.query(`
      UPDATE inf_categories
    ...
    console.error("[Infrastructure] Update category error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Delete category
app.delete("/api/infrastructure/categories/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { rowCount } = await pool.query(`DELETE FROM inf_categories WHERE id = $1`, [id]);
    ...
    console.error("[Infrastructure] Delete category error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
// List items
app.get("/api/infrastructure/items", async (req, res) => {
  try {
    ...
    console.error("[Infrastructure] List items error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Get single item
app.get("/api/infrastructure/items/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(`
      SELECT i.*, c.name as category_name, c.color as category_color, c.icon as category_icon
    ...
    console.error("[Infrastructure] Get item error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Create item
app.post("/api/infrastructure/items", async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      INSERT INTO inf_items (name, code, category_id, building, floor, location, description, notes, data, created_by_email, created_by_name)
    ...
    console.error("[Infrastructure] Create item error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Update item
app.put("/api/infrastructure/items/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { name, code, category_id, building, floor, location, description, notes, data } = req.body;
    ...
    console.error("[Infrastructure] Update item error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Delete item
app.delete("/api/infrastructure/items/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const user = userInfo(req);

    // Check ownership before deletion
    const { rows: itemRows } = await pool.query(`SELECT created_by_email FROM inf_items WHERE id = $1`, [id]);
    ...
    const { rows: files } = await pool.query(`SELECT filepath FROM inf_files WHERE item_id = $1`, [id]);
    for (const f of files) {
      try { await fsp.unlink(f.filepath); } catch {}
    }

    const { rowCount } = await pool.query(`DELETE FROM inf_items WHERE id = $1`, [id]);
    ...
    console.error("[Infrastructure] Delete item error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
// PHOTO UPLOAD
// ====================
app.post("/api/infrastructure/items/:id/photo", uploadAny.single("photo"), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Insert into inf_files with content (kind = 'photo')
    const { rows: ins } = await pool.query(`
      INSERT INTO inf_files (item_id, filename, filepath, mimetype, size_bytes, content, kind)
    ...
    await pool.query(`UPDATE inf_items SET photo_path = $1, photo_file_id = $2, updated_at = now() WHERE id = $3`,
    ...
    console.error("[Infrastructure] Photo upload error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Get photo
app.get("/api/infrastructure/items/:id/photo", async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(`SELECT photo_path, photo_file_id FROM inf_items WHERE id = $1`, [id]);
    ...
    if (row.photo_file_id) {
      const { rows: frows } = await pool.query(
        `SELECT mimetype, content FROM inf_files WHERE id = $1`,
        [row.photo_file_id]
      );
      const f = frows[0];
      if (f?.content) {
    ...
    console.error("[Infrastructure] Get photo error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
// FILES
// ====================
app.get("/api/infrastructure/items/:id/files", async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(`
      SELECT id, item_id, filename, filepath, mimetype, size_bytes, created_at
      FROM inf_files WHERE item_id = $1 AND (kind IS NULL OR kind = 'file')
      ORDER BY created_at DESC
    `, [id]);
    res.json({ ok: true, files: rows });
  } catch (e) {
    console.error("[Infrastructure] List files error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

app.post("/api/infrastructure/items/:id/files", uploadAny.single("file"), async (req, res) => {
  try {
    const { id } = req.params;
    if (!req.file) return res.status(400).json({ ok: false, error: "No file" });
    ...
    const { rows } = await pool.query(`
      INSERT INTO inf_files (item_id, filename, filepath, mimetype, size_bytes, content)
      VALUES ($1, $2, $3, $4, $5, $6)
    ...
    console.error("[Infrastructure] Upload file error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Download file
app.get("/api/infrastructure/files/:id/download", async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(`SELECT filename, filepath, mimetype, content FROM inf_files WHERE id = $1`, [id]);
    ...
    console.error("[Infrastructure] Download file error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

app.delete("/api/infrastructure/files/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query(`DELETE FROM inf_files WHERE id = $1 RETURNING filepath`, [id]);
    ...
    console.error("[Infrastructure] Delete file error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.get("/api/infrastructure/maps/plans", async (_req, res) => {
  try {
    ...
    console.error("[Infrastructure] List plans error:", e);
    res.json({ ok: true, plans: [] });
  }
});

// Get plan file from VSD tables directly (same as GLO - symbiosis pattern)
app.get("/api/infrastructure/maps/plan/:id/file", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error("[Infrastructure] Get plan file error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Get positions for a plan
app.get("/api/infrastructure/maps/positions", async (req, res) => {
  try {
    ...
    console.error("[Infrastructure] Get positions error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.put("/api/infrastructure/maps/positions/:item_id", async (req, res) => {
  try {
    ...
    const deleteResult = await pool.query(
      `DELETE FROM inf_positions WHERE item_id = $1`,
      [item_id]
    );
    console.log(`[Infrastructure] Deleted ${deleteResult.rowCount} old positions for item ${item_id}`);

    // Then insert the new position
    const { rows } = await pool.query(`
      INSERT INTO inf_positions (item_id, logical_name, page_index, x_frac, y_frac)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `, [item_id, logical_name, parseInt(page_index), x_frac, y_frac]);

    console.log(`[Infrastructure] Created new position for item ${item_id} on plan ${logical_name}`);
    ...
    console.error("[Infrastructure] Set position error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Cleanup duplicate positions
app.post("/api/infrastructure/maps/cleanup-duplicates", async (req, res) => {
  try {
    ...
    console.log(`[Infrastructure] Found ${duplicates.length} items with duplicate positions`);

    let totalRemoved = 0;
    for (const dup of duplicates) {
      const result = await pool.query(`
        DELETE FROM inf_positions
        WHERE item_id = $1
        AND id NOT IN (
          SELECT id FROM inf_positions
          WHERE item_id = $1
    ...
    console.error("[Infrastructure] Cleanup error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Delete position
app.delete("/api/infrastructure/maps/positions/:id", async (req, res) => {
  try {
    const { id } = req.params;
    await pool.query(`DELETE FROM inf_positions WHERE id = $1`, [id]);
    res.json({ ok: true });
  } catch (e) {
    console.error("[Infrastructure] Delete position error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Get all placed item IDs
app.get("/api/infrastructure/maps/placed-ids", async (_req, res) => {
  try {
    const { rows } = await pool.query(`
    ...
    console.error("[Infrastructure] Get placed IDs error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
// This allows Infrastructure to display VSD, HV, MECA, GLO, Mobile Equipment, Switchboards markers
app.get("/api/infrastructure/maps/external-positions", async (req, res) => {
  try {
    ...
    } catch (e) { console.log('[Infrastructure] VSD positions not available:', e.message); }
    ...
    } catch (e) { console.log('[Infrastructure] HV positions not available:', e.message); }
    ...
    } catch (e) { console.log('[Infrastructure] MECA positions not available:', e.message); }
    ...
    } catch (e) { console.log('[Infrastructure] GLO positions not available:', e.message); }
    ...
    } catch (e) { console.log('[Infrastructure] Mobile positions not available:', e.message); }
    ...
    } catch (e) { console.log('[Infrastructure] Switchboard positions not available:', e.message); }

    // Datahub items
    try {
      const { rows } = await pool.query(`
        SELECT p.id, p.item_id as equipment_id, p.x_frac, p.y_frac, p.page_index,
               i.name, i.code, i.building, i.floor, c.name as category_name, c.color as category_color
          FROM dh_positions p
          JOIN dh_items i ON i.id = p.item_id
          LEFT JOIN dh_categories c ON c.id = i.category_id
         WHERE p.logical_name = $1 AND p.page_index = $2
      `, [logical_name, pageIdx]);
      result.datahub = rows.map(r => ({
        id: r.id,
        equipment_id: r.equipment_id,
        x_frac: parseFloat(r.x_frac),
        y_frac: parseFloat(r.y_frac),
        name: r.name || r.code || 'Datahub',
        building: r.building,
        floor: r.floor || '',
        details: r.category_name || '',
        color: r.category_color
      }));
    } catch (e) { console.log('[Infrastructure] Datahub positions not available:', e.message); }
    ...
    const totals = {
      ...
      datahub: result.datahub.length
    };

    res.json({ ok: true, positions: result, totals });
  } catch (e) {
    console.error("[Infrastructure] Get external positions error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
// BULK OPERATIONS
// ====================
app.post("/api/infrastructure/bulk/rename", async (req, res) => {
  try {
    ...
    const { rowCount } = await pool.query(
      `UPDATE inf_items SET ${field} = $1, updated_at = now() WHERE ${field} = $2`,
      [to, from]
    );
    ...
    console.error("[Infrastructure] Bulk rename error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
// STATS
// ====================
app.get("/api/infrastructure/stats", async (_req, res) => {
  try {
    const { rows: totals } = await pool.query(`
      SELECT
        (SELECT COUNT(*) FROM inf_items) as total_items,
        (SELECT COUNT(*) FROM inf_categories) as total_categories,
        (SELECT COUNT(DISTINCT item_id) FROM inf_positions) as placed_items
    `);

    const { rows: byCategory } = await pool.query(`
      SELECT c.id, c.name, c.color, c.icon, COUNT(i.id) as item_count
        FROM inf_categories c
        LEFT JOIN inf_items i ON i.category_id = c.id
    ...
    console.error("[Infrastructure] Stats error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// ====================
// REPORT PDF GENERATION - Professional Infrastructure Report
// ====================
app.get("/api/infrastructure/report", async (req, res) => {
  try {
    ...
    // Get categories
    const { rows: categories } = await pool.query(`SELECT * FROM inf_categories ORDER BY sort_order, name`);
    ...
    // Colors
    const colors = {
      primary: '#8B5CF6',    // Purple for Infrastructure
      success: '#059669',
      danger: '#dc2626',
      warning: '#d97706',
      muted: '#6b7280',
    ...
    res.setHeader('Content-Disposition', `attachment; filename="rapport_infrastructure_${new Date().toISOString().split('T')[0]}.pdf"`);
    doc.pipe(res);

    // ========== PAGE DE COUVERTURE ==========
    doc.rect(0, 0, 595, 842).fill('#f5f3ff');
    doc.rect(0, 0, 595, 180).fill(colors.primary);

    doc.fontSize(32).fillColor('#fff').text('RAPPORT INFRASTRUCTURE', 50, 70, { align: 'center', width: 495 });
    doc.fontSize(14).text('Gestion des Infrastructures', 50, 115, { align: 'center', width: 495 });
    ...
    const sommaire = [
      { num: '1', title: 'Presentation de l\'Infrastructure', page: 3 },
    ...
    // ========== 1. PRÉSENTATION ==========
    doc.addPage();
    doc.fontSize(20).fillColor(colors.primary).text('1. Presentation de l\'Infrastructure', 50, 50);
    doc.moveTo(50, 80).lineTo(545, 80).strokeColor(colors.primary).lineWidth(1).stroke();

    let presY = 100;
    doc.fontSize(11).fillColor(colors.text)
       .text('Le module Infrastructure permet de gerer et suivre tous les elements d\'infrastructure du site:', 50, presY, { width: 495 });
    presY += 35;

    const features = [
      'Inventorier les batiments, locaux et zones techniques',
      ...
      'Faciliter la planification et la maintenance',
    ];
    ...
    if (plans.length === 0) {
      doc.fontSize(11).fillColor(colors.muted).text('Aucun plan disponible.', 50, catY, { width: 495 });
    } else {
      doc.fontSize(9).fillColor(colors.muted)
         .text('Les plans utilises sont ceux du module VSD.', 50, catY, { width: 495 });
      catY += 25;
    ...
    if (items.length > 0) {
      ...
      let ficheY = 100;
      doc.fontSize(11).fillColor(colors.muted).text(`${items.length} element(s) Infrastructure`, 50, ficheY);
      ficheY += 30;
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.fontSize(8).fillColor(colors.muted)
         .text(`Rapport Infrastructure - ${site} - Page ${i + 1}/${range.count}`, 50, 810, { align: 'center', width: 495 });
    }

    doc.end();
    console.log(`[Infrastructure] Generated professional PDF with ${totalCount} items`);

  } catch (e) {
    console.error('[Infrastructure] Report error:', e);
