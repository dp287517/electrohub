...
const EQUIPMENT_LABELS = {
  ...
  device: 'Appareil',
  fire_control: 'Asservissement Incendie',
  fire_detector: 'Détecteur Incendie',
  fire_campaign: 'Campagne Incendie'
};
...
function getEquipmentUrl(type, id) {
  const urls = {
    atex: `/app/atex?eq=${id}`,
    vsd: `/app/vsd?vsd=${id}`,
    meca: `/app/meca?meca=${id}`,
    hv: `/app/hv?equipment=${id}`,
    glo: `/app/glo?glo=${id}`,
    mobile: `/app/mobile-equipments?equipment=${id}`,
    door: `/app/doors?door=${id}`,
    switchboard: `/app/switchboards?board=${id}`,
    device: `/app/switchboards?device=${id}`,
    fire_control: `/app/fire-control?check=${id}`,
    fire_detector: `/app/fire-control?detector=${id}`,
    fire_campaign: `/app/fire-control?campaign=${id}`
  };
...
async function sendToUsers(userIds, notification) {
  ...
  try {
    // Use case-insensitive matching for email-based user IDs
    const lowerUserIds = userIds.map(id => id.toLowerCase());
...
async function broadcastExcept(notification, excludeUserId) {
  try {
    // Use case-insensitive comparison for email-based user IDs
    const result = await pool.query(
      'SELECT DISTINCT user_id FROM push_subscriptions WHERE LOWER(user_id) != LOWER($1)',
      [excludeUserId || '']
    );
...
export async function notifyUser(userEmail, title, body, options = {}) {
  ...
  try {
    ...
    // Try to find subscriptions - check by UUID first, then by email
    // (Haleon users may have subscriptions stored with email as user_id)
    // ORDER BY last_used_at DESC to get the most recent first
    // LIMIT to 1 subscription per user to avoid duplicate notifications
    let subsResult;

    if (userId) {
      // First try with UUID from database
      subsResult = await pool.query(`
        SELECT DISTINCT ON (endpoint) endpoint, keys_p256dh, keys_auth, last_used_at
        FROM push_subscriptions
        WHERE user_id = $1
        ORDER BY endpoint, last_used_at DESC NULLS LAST
      `, [userId]);
    }

    // If no subscription found with UUID, try with email directly
    // (Bubble auth stores email as the user_id in JWT)
    ...
    console.log(`[Push] Found ${subsResult.rows.length} subscription(s) for ${userEmail} (type: ${notification.type})`);
    ...
    // Log the URL being sent in the notification for debugging
    if (notification.data?.url) {
      console.log(`[Push] Notification URL: ${notification.data.url}`);
    }
...
export async function notifyNewUserPending(user) {
  ...
  try {
    // Get admin users from the database - check both users and haleon_users tables
    const adminResult = await pool.query(`
      SELECT DISTINCT ps.user_id
      FROM push_subscriptions ps
      LEFT JOIN users u ON ps.user_id::text = u.id::text OR LOWER(ps.user_id) = LOWER(u.email)
      LEFT JOIN haleon_users hu ON ps.user_id::text = hu.id::text OR LOWER(ps.user_id) = LOWER(hu.email)
      WHERE u.role = 'admin'
         OR hu.role = 'admin'
         OR LOWER(COALESCE(u.email, hu.email, ps.user_id)) IN (
    ...
    if (adminUserIds.length === 0) {
      // Try to find admins from admin_emails table (subscriptions stored by email)
    ...
    console.log(`[Push] Found ${adminUserIds.length} admin subscription(s) for pending user notification`);
