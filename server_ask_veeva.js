// server_ask_veeva.js — Ask Veeva (DeepSearch++ v5 ready, intents & guide mode)
// Node ESM
...
// PDF.js (Node): legacy ESM build — NE PAS TOUCHER
...
// -----------------------------------------------------------------------------
// PDF.js (Node): legacy build + standard fonts — NE PAS TOUCHER
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// ZIP streaming & parsers — NE PAS TOUCHER la logique ZIP
// -----------------------------------------------------------------------------
...
async function ingestConcreteFile(absPath, originalName, ext, bytes) {
  ...
  for (let i = 0; i < segs.length; i += EMBED_BATCH) {
    ...
    // spans (light) pour chaque chunk du batch
    const { rows: inserted } = await pool.query(
      ...
      [docId, chunkIndex - batch.length, chunkIndex - 1]
    );
    const spanParams = [];
    const spanValues = [];
...
// -----------------------------------------------------------------------------
// INTENTS — détection FR/EN “quoi faire / help / NC …”
// -----------------------------------------------------------------------------
const INTENTS = {
...
const INTENT_PATTERNS = [
  [INTENTS.WHAT_TO_DO, [
    /\b(que|quoi)\s+faire\b/i, /\bque\s+dois[-\s]?je\s+faire\b/i,
    /\bwhat\s+(should|do)\s+i\s+do\b/i, /\bnext\s+steps?\b/i
  ]],
  [INTENTS.HOW_TO, [/\bcomment\b/i, /\bhow\s+to\b/i, /\bprocedure\b/i, /\bprocédure\b/i]],
  [INTENTS.HELP_ME, [/\bj'ai\s+besoin\s+d[' ]aide\b/i, /\bhelp\b/i, /\bcan\s+you\s+help\b/i]],
  [INTENTS.LOST, [/\bje\s+suis\s+perdu(e)?\b/i, /\bi'?m\s+lost\b/i, /\bnot\s+sure\b/i]],
  [INTENTS.NON_CONFORMITY, [/\bnon\s*[- ]?conformit[é|e]|\bNC\b/i, /\bd[ée]viation\b/i, /\bNCR\b/i, /\bCAPA\b/i, /\bOOS\b/i]],
  [INTENTS.TROUBLESHOOT, [/\bpanne\b/i, /\bincident\b/i, /\banomalie\b/i, /\btrouble(shoot(ing)?)?\b/i, /\bprobl[èe]me\b/i]],
  [INTENTS.COMPLIANCE, [/\bconformit[ée]\b/i, /\bcompliance\b/i, /\baudit\b/i, /\binspection\b/i]],
  [INTENTS.WHO_CONTACT, [/\bqui\s+(contacter|appeler)\b/i, /\bwho\s+to\s+contact\b/i]]
];
function detectIntent(q) {
  for (const [label, regs] of INTENT_PATTERNS) {
    for (const r of regs) if (r.test(q)) return label;
  }
  return INTENTS.GENERIC;
}
function criteriaForIntent(intent, lang) {
  const fr = {
    base: ["pré-requis", "EHS/sécurité", "procédure/étapes", "IPC/contrôles", "tolérances/paramètres", "responsabilités", "enregistrements"],
    nc: ["définition non-conformité", "quarantaine/isolement", "documentation déviation/NCR", "analyse de cause", "actions CAPA", "notifications/qui contacter", "délai/retour en conformité"]
  };
  const en = {
    base: ["prerequisites", "EHS/safety", "procedure/steps", "IPC/controls", "tolerances/parameters", "responsibilities", "records"],
    nc: ["nonconformity definition", "quarantine/isolation", "deviation/NCR documentation", "root cause analysis", "CAPA actions", "notifications/who to contact", "timeline/return to compliance"]
  };
  const ref = (lang === "en") ? en : fr;
  if (intent === INTENTS.NON_CONFORMITY) return [...ref.base, ...ref.nc];
  if ([INTENTS.WHAT_TO_DO, INTENTS.HOW_TO, INTENTS.HELP_ME, INTENTS.TROUBLESHOOT].includes(intent)) return ref.base;
  if (intent === INTENTS.COMPLIANCE) return [...ref.base, (lang==="en"?"audit readiness":"préparation audit")];
  if (intent === INTENTS.WHO_CONTACT) return [(lang==="en"?"who to contact":"qui contacter")];
  return ref.base;
}

// -----------------------------------------------------------------------------
// THREAD MEMORY (global/specific/SOP + topic)
// -----------------------------------------------------------------------------
...
app.post("/api/ask-veeva/pysearch/compare", async (req, res) => {
  try {
    const body = req.body || {};
    const r = await fetch(PYCOMPARE_URL, { method: "POST", headers: { "content-type":"application/json" }, body: JSON.stringify(body) }).then(r=>r.json());
    res.json(r);
  } catch (e) {
    res.status(500).json({ ok:false, error: e.message });
  }
...
// -----------------------------------------------------------------------------
// ASK (DeepSearch++ bridge) — intents + GUIDE mode
// -----------------------------------------------------------------------------
app.post("/api/ask-veeva/ask", async (req, res) => {
  ...
  try {
    ...
    // Intent
    const intent = intent_hint || detectIntent(originalQuestion);
    ...
    if (contextMode === "none") {
      ...
      await pool.query(
        ...
        [userEmail, originalQuestion, Date.now() - t0, { contextMode, intent }]
      ).catch(()=>{});
    ...
    // 3) Fusion & repondération (intent + global/specific + SOP)
    const byKey = new Map();
    for (const s of [...hybrid, ...vectorList]) {
      ...
      // Biais intent “guide”
      if ([INTENTS.WHAT_TO_DO, INTENTS.HOW_TO, INTENTS.HELP_ME, INTENTS.NON_CONFORMITY, INTENTS.TROUBLESHOOT].includes(intent)) {
        if (/\b(checklist|procedure|procédure|sop|policy|guide|instruction)\b/i.test(s.filename)) score += 0.18;
        if (/\b(form|formulaire|record|enregistrement|template|fiche)\b/i.test(s.filename)) score += 0.08;
      }
    ...
    // 4) Si demande explicite de numéro SOP/IDR → réponse directe
    ...
    if (askNum) {
      ...
      if (uniq.length) {
        ...
        await pool.query(
          ...
          [userEmail, originalQuestion, text.length, Date.now() - t0, { mode: "code_lookup", codes: uniq.map(u=>u.code), intent }]
        ).catch(()=>{});
    ...
    // 5) Contexte final (<=3 chunks/doc), prompt & réponse
    const byDoc = new Map();
    ...
    // Optionnel: Evidence matrix via /compare pour intents “guide”
    let evidenceMatrixText = "";
    if (PYSEARCH_ON && [INTENTS.WHAT_TO_DO, INTENTS.HOW_TO, INTENTS.HELP_ME, INTENTS.NON_CONFORMITY, INTENTS.TROUBLESHOOT, INTENTS.COMPLIANCE].includes(intent)) {
      const topic = normalizedQ;
      const doc_ids = contexts.slice(0, 6).map(d => d.doc_id);
      const criteria = criteriaForIntent(intent, qLang);
      try {
        const cmp = await fetch(PYCOMPARE_URL, {
          method: "POST",
          headers: { "content-type":"application/json" },
          body: JSON.stringify({ topic, doc_ids, criteria, k_per_crit: 2, role: user?.role || null, sector: user?.sector || null })
        }).then(r=>r.json());
        if (cmp?.ok && Array.isArray(cmp.matrix)) {
          const lines = [];
          for (const row of cmp.matrix) {
            const crit = row.criterion;
            for (const cell of row.docs) {
              const evs = (cell.evidence || []).map(e => e.text).filter(Boolean).slice(0,2);
              if (evs.length) lines.push(`• [${crit}] ${evs.join(" | ")}`);
            }
          }
          evidenceMatrixText = lines.slice(0, 30).join("\n");
        }
      } catch {}
    }
    ...
    const guideConstraints = (qLang === "en")
      ? [
          "Produce an ACTIONABLE plan with numbered steps.",
          "Include prerequisites, EHS/safety cautions, IPC/controls, tolerances, responsibilities, and required records.",
          "If the case is a non-conformity/deviation: include quarantine/isolation, deviation/NCR record, root cause analysis, and CAPA actions.",
          "When information is missing in CONTEXT, state 'MISSING' and propose the best source SOP/policy to consult."
        ]
      : [
          "Fournis un plan D'ACTION avec étapes numérotées.",
          "Inclure prérequis, précautions EHS/sécurité, IPC/contrôles, tolérances, responsabilités et enregistrements requis.",
          "Si c’est une non-conformité/déviation : inclure quarantaine/isolement, enregistrement déviation/NCR, analyse de cause, et actions CAPA.",
          "Quand une info manque dans le CONTEXTE, indique 'MANQUANT' et propose le SOP/politique cadre à consulter."
        ];
    const isGuideIntent = [INTENTS.WHAT_TO_DO, INTENTS.HOW_TO, INTENTS.HELP_ME, INTENTS.NON_CONFORMITY, INTENTS.TROUBLESHOOT, INTENTS.COMPLIANCE, INTENTS.LOST, INTENTS.WHO_CONTACT].includes(intent);
    ...
    const constraintsBlock = isGuideIntent
      ? (`\n${qLang==="en" ? "Guide constraints" : "Contraintes guide"}:\n- ` + guideConstraints.join("\n- "))
      : "";

    const evidenceBlock = evidenceMatrixText
      ? (`\n${qLang==="en" ? "Evidence (criteria snippets):" : "Évidence (extraits par critère)"}\n` + evidenceMatrixText)
      : "";
    ...
    await pool.query(
      ...
      [userEmail, originalQuestion, { preferGlobal: thread.preferGlobal, preferSOP: thread.preferSOP, lang: qLang, intent }]
    ).catch(()=>{});
    await pool.query(
      `INSERT INTO askv_events(user_email,type,question,answer_len,latency_ms,meta)
       VALUES ($1,'ask_answered',$2,$3,$4,$5)`,
      [userEmail, originalQuestion, text.length, Date.now() - t0, { citations: citations.length, intent }]
    ).catch(()=>{});

    return res.json({ ok: true, text, citations, contexts, intent });
  } catch (e) {
    res.status(500).json({ error: String(e?.message || e) });
  }
});
