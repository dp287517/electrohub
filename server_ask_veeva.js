...
// -----------------------------------------------------------------------------
// INTENTS — détection FR/EN pour “quoi faire / que faire / help / non-conformité…”
// -----------------------------------------------------------------------------
const INTENTS = {
...
// large couverture (sans s’arrêter aux seuls exemples)
const INTENT_PATTERNS = [
  ...
  [INTENTS.HOW_TO, [
    /\bcomment\b/i, /\bhow\s+to\b/i, /\bprocedure\b/i, /\bprocédure\b/i
  ]],
  [INTENTS.HELP_ME, [
    /\bj'ai\s+besoin\s+d[' ]aide\b/i, /\bhelp\b/i, /\bcan\s+you\s+help\b/i
  ]],
  [INTENTS.LOST, [
    /\bje\s+suis\s+perdu(e)?\b/i, /\bi'?m\s+lost\b/i, /\bnot\s+sure\b/i
  ]],
  [INTENTS.NON_CONFORMITY, [
    /\bnon\s*[- ]?conformit[é|e]|\bNC\b/i, /\bd[ée]viation\b/i, /\bNCR\b/i, /\bCAPA\b/i, /\bOOS\b/i
  ]],
  [INTENTS.TROUBLESHOOT, [
    /\bpanne\b/i, /\bincident\b/i, /\banomalie\b/i, /\btrouble(shoot(ing)?)?\b/i, /\bprobl[èe]me\b/i
  ]],
  [INTENTS.COMPLIANCE, [
    /\bconformit[ée]\b/i, /\bcompliance\b/i, /\baudit\b/i, /\binspection\b/i
  ]],
  [INTENTS.WHO_CONTACT, [
    /\bqui\s+(contacter|appeler)\b/i, /\bwho\s+to\s+contact\b/i
  ]]
];
...
function criteriaForIntent(intent, lang) {
  ...
  if (intent === INTENTS.WHAT_TO_DO || intent === INTENTS.HOW_TO || intent === INTENTS.HELP_ME || intent === INTENTS.TROUBLESHOOT)
    return ref.base;
...
app.post("/api/ask-veeva/ask", async (req, res) => {
  ...
  try {
    ...
    // Optionnel: matrice d’évidence via /compare pour les intents “guide”
    ...
    // Mode guide : exigences supplémentaires
...
app.post("/api/ask-veeva/uploadZip", uploadDirect.single("zip"), async (req, res) => {
  try {
    ...
    try {
      ...
      const entries = await zip.entries();
      total = Object.values(entries).filter((e) => !e.is
…(suite du fichier)…
(le message précédent a atteint la limite ; je te colle ici la fin sans rien omettre)
