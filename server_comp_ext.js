...
async function ensureSchema() {
  ...
  try {
    ...
    await c.query(
      `CREATE INDEX IF NOT EXISTS idx_comp_ext_visits_vendor_id ON comp_ext_visits(vendor_id);`
    );
    await c.query(
      `CREATE INDEX IF NOT EXISTS idx_comp_ext_visits_dates ON comp_ext_visits(start_date, end_date);`
    );
...
// NEW: Get single vendor (pour ouvrir depuis calendrier/Gantt)
...
app.put("/api/comp-ext/vendors/:id", async (req, res) => {
  ...
  try {
    ...
    if (set.length) {
      await c.query(
        `UPDATE comp_ext_vendors SET ${set.join(", ")} WHERE id=$${params.length + 1}`,
        [...params, id]
      );
    }
...
// ---------- Calendar (ISO strings enrichies pour calendrier + Gantt + modale) ----------
  ...
  try {
    ...
    for (const r of rows) {
      ...
      tasks.push({
        ...
        // méta pour ouverture modale puis vendor
      ...
      // Événements par jour pour calendrier, avec vendor_name (pas "(v)")
      const ds = new Date(s);
      const de = new Date(e);
