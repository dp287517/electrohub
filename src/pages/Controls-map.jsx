...
function makeTaskIcon(status) {
  ...
  const blink = status === "Overdue" ? "blink-red" : status === "Pending" ? "blink-orange" : "";
...
export default function ControlsMap({
  ...
  const planDisplayName = useMemo(
    () => (plan?.display_name || plan?.logical_name || plan?.id || building || "").toString(),
    [plan, building]
  );
  ...
  return (
    ...
    >
      ...
      {/* Légende statuts */}
      <div className="absolute bottom-3 right-3 z-[1000] bg-white p-3 rounded-xl shadow-lg border text-xs max-w-[200px]">
        <div className="font-semibold mb-2">Statuts</div>
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full" style={{ background: STATUS_COLOR.Planned.fill }} />
            <span className="text-xs">Planifié</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full blink-orange" style={{ background: STATUS_COLOR.Pending.fill }} />
            <span className="text-xs">≤30j</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full blink-red" style={{ background: STATUS_COLOR.Overdue.fill }} />
            <span className="text-xs">Retard</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full" style={{ background: STATUS_COLOR.Done.fill }} />
