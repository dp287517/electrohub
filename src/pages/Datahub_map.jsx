...
const DetailPanel = ({ item, category, position, onClose, onDelete, onNavigate, isMobile, links = [], linksLoading = false, onAddLink, onDeleteLink, onLinkClick, currentPlan, currentPageIndex = 0, mapContainerRef }) => {
  ...
  const hasCustomPosition = !isMobileState && Object.keys(desktopStyle).length > 0;

  return (
    <div ref={panelRef} className={`${hasCustomPosition ? 'bg-white rounded-xl shadow-xl border overflow-hidden animate-slideUp pointer-events-auto flex flex-col' : `${isMobile ? 'fixed inset-x-2 bottom-20 z-[60]' : 'absolute top-4 right-4 w-72 z-[60]'} bg-white rounded-xl shadow-xl border overflow-hidden animate-slideUp pointer-events-auto flex flex-col`}`} style={hasCustomPosition ? desktopStyle : {}}>
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-3 py-2 text-white flex-shrink-0">
      ...
      <div className="p-2 overflow-y-auto flex-1">
        ...
        <div className="mb-2">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <button onClick={() => setShowAddLink(!showAddLink)} className="p-0.5 hover:bg-gray-100 rounded text-gray-500 hover:text-indigo-600" title="Ajouter un lien"><Plus size={14} /></button>
          </div>
          {showAddLink && (
            <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-1.5 mb-1.5">
              <input type="text" value={searchQuery} onChange={(e) => handleSearch(e.target.value)} placeholder="Rechercher..." className="w-full px-2 py-1 text-xs border rounded focus:ring-1 focus:ring-indigo-500 bg-white" autoFocus />
          ...
          {linksLoading ? (
            ...
            <div className="space-y-1 max-h-28 overflow-y-auto">
              {links.map((link, idx) => {
                ...
                return (
                  <div key={link.id || idx} className={`flex items-center justify-between p-1.5 rounded text-xs ${samePlan ? 'bg-indigo-50 border border-indigo-200' : 'bg-gray-50 border border-gray-200'}`}>
                    <button onClick={() => onLinkClick?.(link)} className="flex items-center gap-1.5 flex-1 text-left hover:underline min-w-0">
                      ...
                      {!samePlan && eq?.plan && <span className="text-indigo-600 flex-shrink-0">(autre plan)</span>}
                    </button>
        ...
        <button onClick={() => onNavigate(item)} className="w-full py-2 px-3 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-1.5">
...
export default function DatahubMap() {
  ...
  const drawConnections = useCallback(() => {
    ...
    links.forEach((link) => {
      ...
      else if (linkLabel === 'powers') { color = '#f59e0b'; hasDirection = true; }
      else if (linkLabel === 'powered_by') { color = '#f59e0b'; hasDirection = true; swapDirection = true; }
