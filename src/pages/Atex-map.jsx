...
function Input({ value, onChange, className = "", ...p }) {
  return (
    <input
      className={`border rounded-lg px-3 py-2 text-sm w-full focus:ring focus:ring-blue-100 bg-white text-black placeholder-black disabled:bg-gray-50 disabled:text-gray-500 ${className}`}
      value={value ?? ""}
...
export default function Atex() {
  ...
  // 💡 Dernier contrôle => Prochain contrôle = +36 mois (auto)
  function onChangeLastCheck(dateStr) {
    if (!editing) return;
    const next = dateStr ? dayjs(dateStr).add(36, "month").format("YYYY-MM-DD") : "";
    setEditing({ ...editing, last_check_date: dateStr || null, next_check_date: next || null });
  }
  ...
  async function saveBase() {
    ...
    const payload = {
      name: editing.name || "",
      // Bâtiment & Zone: hérités du plan (lecture seule ici) → on renvoie ce qu'on a déjà
      ...
      last_check_date: editing.last_check_date || null, // ✅ nouveau
      next_check_date: editing.next_check_date || null, // auto mis à jour si last_check_date change
      ...
      {drawerOpen && editing && (
        <Drawer title={`ATEX • ${editing.name || "nouvel équipement"}`} onClose={closeEdit}>
          <div className="space-y-4">
            ...
            {/* Localisation (lecture seule — héritée du plan) */}
            <div className="grid sm:grid-cols-2 gap-3">
              <Labeled label="Bâtiment (depuis le plan)">
                <Input value={editing.building || ""} onChange={() => {}} disabled />
              </Labeled>
              <Labeled label="Zone (depuis le plan)">
                <Input value={editing.zone || ""} onChange={() => {}} disabled />
              </Labeled>
            </div>

            {/* Macro / sous-équipement (renseignés par le plan et les sous-zones) */}
            <div className="grid sm:grid-cols-2 gap-3">
              <Labeled label="Équipement (macro, nom du plan)">
                <Input value={editing.equipment || ""} onChange={() => {}} disabled />
              </Labeled>
              <Labeled label="Sous-Équipement (nom de la sous-zone)">
                <Input value={editing.sub_equipment || ""} onChange={() => {}} disabled />
              </Labeled>
            </div>

            {/* Zonage (modifiable si besoin) */}
            <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <Labeled label="Dernier contrôle (modifiable)">
                <Input
                  type="date"
                  value={asDateInput(editing.last_check_date)}
                  onChange={onChangeLastCheck}
                />
              </Labeled>
            </div>

            <div className="grid sm:grid-cols-2 gap-3">
              <Labeled label="Prochain contrôle (auto = +36 mois)">
                <Input
                  type="date"
