...
export const api = {
  ...
  // Dans la section VSD de api.js, ajouter/corriger :
...
vsd: {
  ...
  getEquipment: (id) => get(`/api/vsd/equipments/${encodeURIComponent(id)}`),
  createEquipment: (payload) => post("/api/vsd/equipments", payload),
  updateEquipment: (id, payload) => put(`/api/vsd/equipments/${encodeURIComponent(id)}`, payload),
  deleteEquipment: (id) => del(`/api/vsd/equipments/${encodeURIComponent(id)}`),

  // Photo principale
  photoUrl: (id, { bust = true } = {}) =>
    withBust(`${API_BASE}/api/vsd/equipments/${encodeURIComponent(id)}/photo`, bust),
  uploadPhoto: (id, file) => {
    const fd = new FormData();
    fd.append("photo", file);
    return upload(`/api/vsd/equipments/${encodeURIComponent(id)}/photo`, fd);
  ...
  // Fichiers attachés
  listFiles: (equipmentId) => get(`/api/vsd/equipments/${encodeURIComponent(equipmentId)}/files`),
  uploadFiles: (equipmentId, files = []) => {
    const fd = new FormData();
    (files || []).forEach((f) => fd.append("files", f));
    return upload(`/api/vsd/equipments/${encodeURIComponent(equipmentId)}/files`, fd);
  },
  deleteFile: (id) => del(`/api/vsd/files/${encodeURIComponent(id)}`),
  ...
  extractFromPhotos: (files = []) => {
    const fd = new FormData();
    (files || []).forEach((f) => fd.append("files", f));
    return upload(`/api/vsd/ai/analyzePhotoBatch`, fd);
  },
  analyzePhotoBatch: (files = []) => api.vsd.extractFromPhotos(files),
},
...
vsdMaps: {
  ...
  listPlans: () => get(`/api/vsd/maps/plans`),
  renamePlan: (logical_name, display_name) =>
    put(`/api/vsd/maps/plan/${encodeURIComponent(logical_name)}/rename`, { display_name }),
  planFileUrl: (logical_name, { bust = true } = {}) =>
    withBust(`${API_BASE}/api/vsd/maps/plan/${encodeURIComponent(logical_name)}/file`, bust),
  planFileUrlById: (id, { bust = true } = {}) =>
    withBust(`${API_BASE}/api/vsd/maps/plan-id/${encodeURIComponent(id)}/file`, bust),
  planFileUrlAuto: (plan, { bust = true } = {}) => {
    const key = typeof plan === "string" ? plan : plan?.id || plan?.logical_name || "";
    const url =
      isUuid(key) || isNumericId(key)
        ? `${API_BASE}/api/vsd/maps/plan-id/${encodeURIComponent(key)}/file`
        : `${API_BASE}/api/vsd/maps/plan/${encodeURIComponent(key)}/file`;
    return withBust(url, bust);
  },
  positions: (logical_name, page_index = 0) =>
    get(`/api/vsd/maps/positions`, { logical_name, page_index }),
  positionsAuto: (planOrKey, page_index = 0) => {
    ...
    if (isUuid(key) || isNumericId(key))
      return get(`/api/vsd/maps/positions`, { id: key, page_index });
    return get(`/api/vsd/maps/positions`, { logical_name: key, page_index });
  },
  setPosition: (equipmentId, payload) =>
    put(`/api/vsd/maps/positions/${encodeURIComponent(equipmentId)}`, payload),
},
