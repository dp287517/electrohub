...
app.get("/api/projects/projects", async (req, res) => {
  try {
    ...
    const st = await pool.query(
      `SELECT * FROM pm_status WHERE site=$1 AND project_id = ANY($2)`,
      [site, ids]
    );
...
app.post("/api/projects/projects", async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO pm_status(project_id, site) VALUES ($1,$2) ON CONFLICT DO NOTHING`,
      [r.rows[0].id, site]
    );
    await logAudit(site, r.rows[0].id, "create_project", {
      title,
      wbs_number,
      budget_amount,
    });
...
app.put("/api/projects/projects/:id", async (req, res) => {
  try {
    ...
    const { title, wbs_number, budget_amount, prep_month, start_month, close_month } =
      req.body || {};
    const r = await pool.query(
      `
...
app.get("/api/projects/projects/:id/status", async (req, res) => {
  try {
    ...
    const id = Number(req.params.id);
    const { rows } = await pool.query(
      `SELECT * FROM pm_status WHERE site=$1 AND project_id=$2`,
      [site, id]
    );
...
app.post("/api/projects/projects/:id/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    await logAudit(site, id, "upload_file", {
      category,
...
app.get("/api/projects/download", async (req, res) => {
  try {
    ...
    const q = await pool.query(
      `SELECT filename, mime, file FROM pm_files WHERE id=$1 AND site=$2`,
      [fileId, site]
    );
...
app.get("/api/projects/projects/:id/lines", async (req, res) => {
  try {
    ...
    const offers = (
      await pool.query(
        `SELECT * FROM pm_offers  WHERE site=$1 AND project_id=$2 ORDER BY received_at DESC`,
        [site, id]
      )
    ).rows;
    const orders = (
      await pool.query(
        `SELECT * FROM pm_orders  WHERE site=$1 AND project_id=$2 ORDER BY ordered_at DESC`,
        [site, id]
      )
    ).rows;
    const invoices = (
      await pool.query(
        `SELECT * FROM pm_invoices WHERE site=$1 AND project_id=$2 ORDER BY invoiced_at DESC`,
        [site, id]
      )
    ).rows;
...
app.get("/api/projects/projects/:id/analysis", async (req, res) => {
  try {
    const site = siteOf(req);
    const id = Number(req.params.id);
    const pQ = await pool.query(`SELECT * FROM pm_projects WHERE id=$1 AND site=$2`, [
      id,
      site,
    ]);
    ...
    await pool
      .query(`UPDATE pm_status SET last_analysis = $1 WHERE project_id=$2 AND site=$3`, [
        out,
        id,
        site,
      ])
...
app.post("/api/projects/projects/:id/assistant", async (req, res) => {
  try {
    ...
    const pQ = await pool.query(
      `SELECT title, wbs_number, budget_amount FROM pm_projects WHERE id=$1 AND site=$2`,
      [id, site]
    );
    const sums = (
      await pool.query(
      ...
      messages: [
        {
          role: "system",
          content:
            "Tu es un assistant senior PMO/Contrôle de gestion. Donne des conseils concis, actionnables et chiffrés.",
        },
        {
          role: "user",
          content: `${ctx}\nQuestion: ${
            question || "Donne une analyse des risques court terme."
          }`,
        },
      ],
    });
...
/* ==========================================================
 * START — attendre le schéma puis écouter
 * ========================================================== */
...
ensureSchema()
  ...
  .catch((e) => {
    console.error("[PM SCHEMA] init failed:", e);
    process.exit(1);
  });
